...
// Reference to the paragraph for displaying the ball count
const ballCountParagraph = document.getElementById('ballCount');
let ballCount = 0; // Initialize ball count

function evilLoop() {
    ctx.fillStyle = 'rgba(0, 0, 0, 0.25)';
    ctx.fillRect(0, 0, width, height);

    ballCount = 0; // Reset ball count every frame before recounting
    ...
    for (const ball of balls) {
        if (ball.exists) {
            ...
            ballCount++; // Only count existing balls
        }
    }
...
// Initialize ball count and start the loop
